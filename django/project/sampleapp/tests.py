...
class ApiTests(TestCase):
    ...
    def test_get_existing_entity(self):
        ...
        response = self.client.get(f'/entity/{e.pk}/')
        self.assertEqual(200, response.status_code)
        data = json.loads(response.content)
        self.assertEqual(data['data']['type'], "entities")
        self.assertEqual(data['data']['id'], e.pk)
        self.assertEqual(
            data['data']['attributes']['sentence'],
            'Hello Boston!')
        self.assertEqual(
    ...
    def test_delete_existing_entity(self):
        e = Entity.objects.create(sentence='Hello Boston!')
        response = self.client.delete(f'/entity/{e.pk}/')
    ...
    def test_only_deletes_what_was_requested(self):
        ...
        response = self.client.delete(f'/entity/{first.pk}/')
        self.assertEqual(204, response.status_code)
    ...
    def test_updates_existing_entity(self):
        ...
        data = json.loads(response.content)
        self.assertEqual(data['data']['type'], "entities")
        self.assertEqual(data['data']['id'], first.pk)
        self.assertEqual(
            data['data']['attributes']['sentence'],
            'Goodbye Des Moines!')
        self.assertEqual(
    ...
    def test_get_list_of_all_entities(self):
        ...
        response = self.client.get(f'/entity/')
        self.assertEqual(200, response.status_code)
        data = json.loads(response.content)['data']
        self.assertEqual(2, len(data))
        self.assertEqual(data[0]['type'], "entities")
        self.assertEqual(data[1]['type'], "entities")
        self.assertEqual(data[0]['id'], first.pk)
        self.assertEqual(data[1]['id'], second.pk)
        self.assertEqual(data[0]['attributes']['sentence'], first.sentence)
        self.assertEqual(data[1]['attributes']['sentence'], second.sentence)
        self.assertEqual(data[0]['attributes']['results'], [{'ent': 'Boston', 'label': 'Location'}])
        self.assertEqual(data[1]['attributes']['results'], [{'ent': 'Des Moines', 'label': 'Location'}])
